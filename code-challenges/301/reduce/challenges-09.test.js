...
const returnNames = (arr) => arr.reduce((accumulator, value) => {
...
const countNumberOfChildren = (arr) => arr.reduce((count, character) => {
  let children = character.children ? character.children.length : 0;
  count += children;
  return count;
}, 0);
...
const calculateAverage = (arr) => arr.reduce((acc, number) => {
  acc.count += 1;
  acc.sum += number;
  acc.ave = acc.sum / acc.count;
  return acc;
}, { count: 0, sum: 0, ave: 0 }).ave;
...
const extractStat = (statName, arr) => arr.reduce((acc, currentStat) => {
  currentStat.stat.name === statName ? acc = currentStat : null;
  return acc;
}, {});
